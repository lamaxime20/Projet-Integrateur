...
### **Implémentation d'un ESP32 pour le pilotage automatique d'une serre connectée via MQTT** (PHAREL)

Au départ, tu m'as demandé de créer un code Arduino destiné à un **ESP32 Dev Module** qui, dès le démarrage, récupère un **token d'authentification** auprès d'un backend via une requête HTTP POST, puis se connecte de manière sécurisée (TLS) à un broker **MQTT HiveMQ Cloud**. Ce code devait intégrer la lecture de **5 capteurs** — luminosité (LDR), humidité du sol (YL-69), température (DHT22), taux de CO2 (SEN0159) et niveau d'eau (flotteur) — et contrôler **4 actionneurs** : une ampoule, une pompe, un ventilateur et un servomoteur (porte), chacun activé ou désactivé selon des seuils min/max définis pour chaque grandeur. Tu m'as précisé que ces seuils devaient avoir des **valeurs par défaut** dans le code, mais que la carte devait rester en **écoute permanente** du broker MQTT pour les mettre à jour en temps réel dès que le backend en envoie de nouveaux. Tu m'as aussi demandé de m'appuyer sur ton fichier `Test.ino` pour la partie WiFi/token/MQTT, et sur ton `Code.ino` pour les pins et la logique capteurs/actionneurs. En réponse, j'ai **fusionné ces deux fichiers** en un seul code structuré et commenté, en ajoutant la structure `Seuils`, le callback MQTT pour la mise à jour dynamique des seuils, toute la logique d'activation des actionneurs selon tes règles exactes (y compris la sécurité qui coupe la pompe si le réservoir est vide), et la publication des mesures toutes les 30 secondes sur le topic MQTT `data`. Pour que le code compile correctement, tu as également dû installer manuellement **5 bibliothèques** depuis le gestionnaire de bibliothèques de l'IDE Arduino : **PubSubClient** de Nick O'Leary pour la communication MQTT, **ArduinoJson** de Benoit Blanchon pour parser et construire les messages JSON, **DHT sensor library** d'Adafruit pour lire le capteur de température et d'humidité DHT22, **Adafruit Unified Sensor** qui est une dépendance obligatoire de la bibliothèque DHT, et enfin **ESP32Servo** de Kevin Harrington pour contrôler le servomoteur depuis l'ESP32.