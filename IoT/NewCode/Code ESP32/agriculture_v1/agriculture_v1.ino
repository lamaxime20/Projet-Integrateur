...
// ============================================================
//  TIMER DE PUBLICATION DES DONNÉES
// ============================================================
unsigned long lastMsg  = 0;
const long    interval = 30000;  // 30 secondes
...
void loop() {
  ...
  if (now - lastMsg >= interval) {
    ...
    Serial.println("\n======================================");
    Serial.println("  Nouvelle lecture des capteurs");
    Serial.println("======================================");

    // ——— Lecture YL-69 : Humidité du sol ———
    int   sol_brut     = analogRead(SOIL_PIN);
    float humidite_sol = map(sol_brut, 4095, 0, 0, 100);
    ...
    float luminosite = map(ldr_brut, 0, 4095, 0, 100);
    Serial.printf("☀️  Luminosité   : %.1f %%\n", luminosite);

    // ——— Lecture SEN0159 : CO2 ———
    int   co2_brut = analogRead(CO2_PIN);
    float co2      = map(co2_brut, 0, 4095, 0, 100);
    Serial.printf("🌫️  CO2 (relatif): %.1f %%\n", co2);

    // ——— Lecture flotteur : Niveau eau ———
    int niveau_eau = digitalRead(WATER_PIN);
    Serial.print("🪣 Niveau eau   : ");
    Serial.println(niveau_eau == HIGH ? "OK" : "Bas !");

    // ——— Application des actionneurs selon les seuils ———
    appliquerActionneurs(luminosite, humidite_sol, temperature, co2, niveau_eau);

    // ——— Publication des mesures vers le broker ———
